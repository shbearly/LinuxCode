...
#include <string.h>
#include <semaphore.h>
#include "memdata_lib.h"   // internal interface
#include "memdata.h"       // external interface used by users

/*
 * Global data structures
 */
memdata_shm_ctx *memdata_ctx = NULL;
int            memdata_initialized = 0;
...
/******************************************************************************
 *                External memdata library access apis
 *****************************************************************************/
 ...
 */
int memdata_get (const char *ns, const char *name, char *out_val, int outbufsz)
{
    if (0 == memdata_initialized || NULL == name || NULL == out_val) {
        return -1;
    }

    char *val = _memdata_get(ns, name);
    if (val) {
...
{
    if (0 == memdata_initialized || NULL == memdata_ctx) {
    ...
    return _memdata_set(ns, name, value, 0);
}
...
int memdata_set_encrypt (const char *ns, const char *name, const char *value)
{
    ...
    return _memdata_set(ns, name, value_encrypt, 0);
}
 ...
 */
int memdata_getall (char *buf, int bufsz, int *outsz)
{
    if (0 == memdata_initialized) {
        return ERR_NOT_INITIALIZED;
    }
    if (NULL == buf) {
        return ERR_INVALID_PARAM;
    ...
    *outsz = _memdata_getall(buf, bufsz);
    return 0;
}
...
int memdata_unset (const char *ns, const char *name)
{
    ...
    return _memdata_unset(ns, name, 0);
}
...
{
    if (0 == memdata_initialized || NULL == name || NULL == value || NULL == out_match) {
        return -1;
    }
...
int memdata_getsz (long int *used_sz, long int *max_sz)
{
    ...
    return _memdata_getsz(used_sz, max_sz);
}
...
int memdata_commit ()
{
    memdata_shm_ctx *ctx = memdata_ctx;
    int rc;

    if (0 == memdata_initialized || NULL == ctx) {
        return ERR_NOT_INITIALIZED;
    }
...
{
    if (memdata_initialized) {
        _memdata_destroy();
        memdata_initialized = 0;
    }
}
...
int memdata_create (const char *file, long int max_file_sz, const char *mtd_device)
{
    ...
    memdata_ctx = (memdata_shm_ctx *) memdata_shm_create(store_info, &shmid);
    if (NULL == memdata_ctx) {
...
{
    //ulog_init();
    //ulog_LOG_Dbg("entering memdata_init");

    if (memdata_initialized) {
        return 0;
    }

    memdata_shm_ctx *ctx = NULL;
    int rc = memdata_shm_init(&ctx);
    if (rc || NULL == ctx) {
...
static int lock_init (memdata_shm_ctx *ctx)
{
    shm_cb *cb = &(ctx->cb);
...
static int lock_destroy (memdata_shm_ctx *ctx)
{
...
static int rw_lock (memdata_shm_ctx *ctx)
{
...
static int rw_unlock (memdata_shm_ctx *ctx)
{
    read_unlock(ctx);
    write_unlock(ctx);
    return 0;
...
/******************************************************************************
 *                Internal memdata library access apis
 *****************************************************************************/
...
static void _memdata_destroy ()
{
    ...
    memdata_shm_destroy(memdata_ctx);
    memdata_ctx = NULL;
}
 ...
 *  Returns
 *         1 - if memdata storage has enough free space to store the tuple
 ...
 *         
 */
static int check_decr_store_sz (memdata_shm_ctx *ctx, const char *name, const char *value)
{
    shm_cb *cb = &(ctx->cb);
    int entry_sz = 0;
...
static void incr_store_sz (memdata_shm_ctx *ctx, const char *name, const char *value)
{
    shm_cb *cb = &(ctx->cb);
    int entry_sz = 0;
...
static int _memdata_set (const char *ns, const char *name, const char *value, int nolock)
{
    ...
    if (0 == strcmp(HT_ENTRY_NAME(ctx,entryoffset), name_p)) {
         ...
         */
        _memdata_unset(ns, name, 1);
        rc = _memdata_set(ns, name, value, 1);
    } else { 
...
static void shm_mm_init (memdata_shm_ctx *ctx);

/*
 * initialize control block
 */
static int shm_cb_init (memdata_shm_ctx *ctx, int shmid, store_info_t store_info)
{
    shm_cb *cb = &(ctx->cb);
...
static int memdata_shm_init (memdata_shm_ctx **out_ctx)
{
    int rc, shmid;
    struct stat sbuf;
    ...
    memdata_shm_ctx *ctx = (memdata_shm_ctx *) memdata_shm_attach(&shmid);
    if (NULL == ctx) {
...
static void *memdata_shm_create (store_info_t store_info, int *out_shmid)
{
    ...
    if (-1 != stat(SYSCFG_SHM_FILE, &sbuf)) {
        void *p_shm = memdata_shm_attach(&shmid);
        if (p_shm) {
    ...
    // attach to shm to initialize memdata structures
    memdata_shm_ctx *ctx = shmat(shmid, NULL, 0);
    if (NULL == ctx) {
        return NULL;
    }
...
static void *memdata_shm_attach (int *shmid)
{
    void *shm_p;

    if (-1 == (*shmid = memdata_shm_getid())) {
        return NULL;
    }
 ...
 */
static void _memdata_file_lock (int fd)
{
    struct flock fl;
 ...
 * Note: fd should be opened with RDWR mode
 */
static void _memdata_file_unlock (int fd)
{
    struct flock fl;
...
int load_from_file (const char *fname)
{
    ...
    buf = inbuf;
    do {
        buf = memdata_parse(buf, &name, &value);
        if (name && value) {
            memdata_set(NULL, name, value);
            free(name);
            free(value);
        }
...
/*
 * Notes
 *    memdata space is locked by the caller (for write & commit)
 */
int commit_to_file (const char *fname)
{
    int fd;
