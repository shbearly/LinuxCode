...
static void memdata_test_basicapi(int dataset)
{
    int i, rc = 0;
    char val[512];
    ...
    if (0 != (rc = memdata_init("junk", NULL))) {
        printf("%s: memdata_init failed (rc=%d)\n", __FUNCTION__, rc);
     
    }
    // set test
    for(i=0; data_arr[i]; i+=2) {
        rc = memdata_set(NULL, data_arr[i], data_arr[i+1]);
        if (0 != rc) {
            printf("%s: memdata_set failed (rc=%d) for %s=%s\n", __FUNCTION__, rc, data_arr[i], data_arr[i+1]);
        } else {
            printf("%s: memdata_set success (rc=%d) for %s=%s\n", __FUNCTION__, rc, data_arr[i], data_arr[i+1]);
        }
    }
    // get test
    for(i=0; data_arr[i]; i+=2) {
        val[0] = '\0';
        rc = memdata_get(NULL, data_arr[i], val, sizeof(val));
        if (strcmp(val,data_arr[i+1])) {
            printf("%s: memdata_get(%s) failed (rc=%d), val=%s\n", __FUNCTION__, data_arr[i], rc, val);
        } else {
            printf("%s: memdata_get(%s) success val=%s\n", __FUNCTION__, data_arr[i], val);
        }
    }
    // replace value test
    for(i=0; data_arr[i]; i+=2) {
        rc = memdata_set(NULL, data_arr[i], data_arr[i+1]);
        if (0 != rc) {
            printf("%s: (replace) memdata_set failed (rc=%d) for %s=%s\n", __FUNCTION__, rc, data_arr[i], data_arr[i+1]);
        } else {
            printf("%s: (replace) memdata_set success (rc=%d) for %s=%s\n", __FUNCTION__, rc, data_arr[i], data_arr[i+1]);
        }
    }
    ...
    // unset test
    for(i=0; data_arr[i]; i+=2) {
        rc = memdata_unset(NULL, data_arr[i]);
        if (0 != rc) {
            printf("%s: memdata_unset failed (rc=%d) for %s=%s\n", __FUNCTION__, rc, data_arr[i], data_arr[i+1]);
        } else {
            printf("%s: memdata_unset success for %s\n", __FUNCTION__, data_arr[i]);
        }
    }
    // get unknown test
    for(i=0; data_arr[i]; i+=2) {
        ...
        rc = memdata_get(NULL, data_arr[i], val, sizeof(val));
        if (0 != rc) {
            printf("%s: memdata_get(%s) unknown failed (rc=%d) val=%s\n", __FUNCTION__, data_arr[i], rc, val);
        } else {
            printf("%s: memdata_get(%s) success val=%s\n", __FUNCTION__, data_arr[i], val);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    if (0 == strcasecmp(argv[1], "1")) {
        for (i = 0; i < repeat; i++)
            memdata_test_basicapi(1);
    } else if (0 == strcasecmp(argv[1], "2")) {
        for (i = 0; i < repeat; i++)
            memdata_test_basicapi(2);
    }
    return rc;
}
